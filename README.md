...
#   GET  /feasibility?reservation_id=N  -> read-only berth-feasibility oracle: discrete,
#                            vessel-sized candidate berths a requested vessel can take (free space
#                            minus any placed plan overlapping the window, snapped to the berth
#                            catalog, depth-checked per footprint). Advises; never places ->
#                            operator picks one in the "Find berth" picker and confirms on select
